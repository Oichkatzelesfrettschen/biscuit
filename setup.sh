...
if command -v pip3 >/dev/null; then
	pip3 install --user --upgrade mypy flake8 pytest breathe >/dev/null 2>&1 || true
fi
...
##
# \brief Build project documentation using Doxygen and Sphinx.
#
# This function generates API documentation from the source tree using
# Doxygen and then converts it into HTML with Sphinx so that the
# resulting files are ready for publication on Read the Docs.
build_docs() {
